...
import { Box3, BoxGeometry, Group, MeshStandardMaterial, Vector3 } from "three";
import { createBuildingFactory } from "./createBuilding";
import { ARCHITECTURES, buildingStyles, styleIndex } from "./buildingStyles";
...
test("all architectural styles keep decorations inside the exact floor-height envelope", () => {
  for (const { id } of ARCHITECTURES) {
    const styles = buildingStyles(id);
    for (let index = 0; index < styles.length; index++) {
      let key = "design";
      while (styleIndex(key, styles.length) !== index) key += "x";
      for (const floors of [1, 5, 1000]) {
        const box = new BoxGeometry(1, 1, 1);
        const factory = createBuildingFactory(box, id);
        ...
        assert.equal(building.userData.styleName, styles[index].name);
        const wallHeight = new Box3()
          .setFromObject(building)
          .getSize(new Vector3()).y;
        assert.ok(
          Math.abs(
            building.geometry.getAttribute("uv").getY(0) -
              wallHeight / FLOOR_HEIGHT,
          ) < 0.0001,
        );
        assert.ok(
          group.children.length <= 5,
          "Details are batched by shared geometry and material",
        );
        factory.dispose();
        box.dispose();
      }
